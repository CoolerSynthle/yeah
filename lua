...
local function LoadFromUrl(x)
	local BASE_USER = "playvoras"
	local BASE_BRANCH = "main"
	local BASE_URL = "https://raw.githubusercontent.com/%s/Fork-Advanced-Decompiler-V3/%s/%s.lua"
				...
				if nextIsAux then
					...
					local opConstructors = {} do
						...
						opConstructors["FASTCALL3"] = function()
							local sR = bit32.band(aux, 0xFFFF)
							local shiftS = bit32.rshift(sR, 8)
							protoOutput ..= `FASTCALL[{Luau:GetBuiltinInfo(A)}]({modifyRegister(B)}, {modifyRegister(bit32.band(sR, 0xFF))}, {modifyRegister(shiftS)})`
						end
						...
						opConstructors["FORNPREP"] = function(insnIndex, A, sD, proto, protoOutput, modifyRegister)
							local initValue = modifyRegister(A + 2)
							local limitValue = modifyRegister(A)
							local stepValue = modifyRegister(A + 1)

							local stepString = (stepValue == "1" and "") or (", " .. stepValue)

							protoOutput ..= string.format("for %s = %s, %s%s do escape at #%i", 
								initValue, limitValue, stepValue, stepString, insnIndex + sD)
						end
						...
						opConstructors["ADD"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." + "..modifyRegister(c))
						end
						opConstructors["ADDK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." + "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						opConstructors["SUB"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." - "..modifyRegister(c))
						end
						opConstructors["SUBK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." - "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						opConstructors["MUL"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." * "..modifyRegister(c))
						end
						opConstructors["MULK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." * "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						opConstructors["DIV"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." / "..modifyRegister(c))
						end
						opConstructors["DIVK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." / "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						opConstructors["MOD"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." % "..modifyRegister(c))
						end
						opConstructors["MODK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." % "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						opConstructors["POW"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." ^ "..modifyRegister(c))
						end
						opConstructors["POWK"] = function(a, b, c)
							protoOutput ..= baseLocal(a, modifyRegister(b).." ^ "..handleConstantValue(proto.constsTable[c + 1] or nilValue))
						end
						...
						opConstructors["DUPTABLE"] = function(a, d)
							local t = proto.constsTable[d + 1].value
							protoOutput ..= baseLocal(a, "{\n")
							for i = 1, #t.keys do
								local id = t.keys[i]
								local k = proto.constsTable[id + 1]
						...
						opConstructors["SETLIST"] = function()
							...
							if count == 0 then -- MULTRET
								-- return case
								protoOutput ..= string.format("%s[%i] = select(%i, ...)\n", modifyRegister(reg), arrayIndex, arrayChunkReg + 1)
							else
								for i = 0, count - 2 do
...
_ENV.decompile = function(script)
	...
	local cleanedOutput = removedstupidnumber(output)

	if RETURN_ELAPSED_TIME then
		return cleanedOutput, elapsedTime
	else
